...
export default function AdminDashboard({ user, onLogout }) {
    ...
    function toggleAllUsers() { const deletableIds = (data?.users ?? []).filter(item => !item.isDeleted).map(item => item.userId); setSelectedUserIds(current => current.length === deletableIds.length ? [] : deletableIds); }
...
function UserTable({ users = [], selectedUserIds = [], onSelect, onToggle, onToggleAll }) { const deletableUsers = users.filter(item => !item.isDeleted); const allSelected = deletableUsers.length > 0 && deletableUsers.every(item => selectedUserIds.includes(item.userId)); return <div className="admin-table"><div className="admin-table-header"><label><input type="checkbox" aria-label="Select all users" checked={allSelected} onChange={onToggleAll} /> Select all</label><span>Name</span><span>Email</span><span>Role</span><span>Companies</span><span>Uploads</span><span>Storage</span><span>Status</span></div>{users.map(item => <div className="admin-user-row" key={item.userId} role="button" tabIndex="0" onClick={() => onSelect(item.userId)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") onSelect(item.userId); }}><input type="checkbox" aria-label={`Select ${item.userName}`} checked={selectedUserIds.includes(item.userId)} disabled={item.isDeleted} onClick={event => event.stopPropagation()} onChange={() => onToggle(item.userId)} /><strong>{item.userName}</strong><span>{item.email}</span><span>{item.role}</span><span>{item.companies?.length ? item.companies.map(company => `${company.companyName} - ${company.accessRole}`).join(", ") : "No company access"}</span><span>{item.uploadsUsed} / {item.uploadLimit}</span><span>{item.storageUsedMb} / {item.storageLimitMb ?? "-"} MB</span><span>{item.isDeleted ? "Deleted" : item.isActive ? "Active" : "Inactive"}</span></div>)}</div>; }
